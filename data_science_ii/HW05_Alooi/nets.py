...
def write_edgelist(Graph, filename, delimiter="\t"):
    ...
    for n in Graph.G:
        ...
        for ne in Graph.G[n]:
            ...
            network_file.write(''.join([n,delimiter,ne,'\n']))
...
def read_edgelist(filename, delimitedby="\t"):
    ...
    for ln, line in enumerate(read_net):
        ...
        if li.startswith("#"):
            ...
            node_edge = net_line.split(delimitedby)[0:2]
            #print(node_edge[0])
...
class Graph(object):
    ...
    def add_node(self, node):
        ...
        the first element"""
        # check to see if the node to be checked was inputted as a list
        # Also if there is more than one entry in the list use only the 
    ...
    def add_edge(self, edge_set):
        ...
        if self.has_edge(edge_set) == False:
            self.G[edge_set[0]].append(edge_set[1])
            # try and detect an orphaned node. Add any orphans to the node list
            # make sure it gets the corresponding edge as well
            if self.has_node(edge_set[1]) == False:
                self.add_node(edge_set[1])
            elif self.has_edge([edge_set[1],edge_set[0]]) == False:
                self.G[edge_set[1]].append(edge_set[0])
            return(True)
    ...
    ############################################################
    def connected_nodes(self, source):
    ...
    ############################################################
    def relabel_nodes(self, N_names):
        ...
        if len(self.G) != len(N_names):
            ...
            original_nodels = list(self.G.keys())
            ...
            for n_n, o_n in enumerate(original_nodels):
                ...
                for e_n in original_nodels:
                    ...
                    #print(org_edges)
                    self.G[e_n] = org_edges
                # replace the key
                self.G[N_names[n_n]] = self.G.pop(o_n)
    ...
    def subgraph(self, Nodes):
        ...
        1) self - this is a Graph() object specified in this module
...
#########################################################################################
#C = read_edgelist("NCAA_2005.edgelist", delimitedby='" "')
#write_edgelist(C, "NCAA_edgelist_2005", delimiter=",")
